...
Mas_Active_Late <- SOT_Master %>% filter(Parent_Vendor == levels(Parent_Vendor)[223], 
  ...
  summarise("SOT_Units" = sum(Units)) %>% 
    arrange( desc(SOT_Units))
datatable(format(Mas_Active_Late, big.mark = ","), extensions = 'Buttons', options = list(pageLength = 5, dom = 'Bfrtip' , buttons = c('excel', 'pdf', 'csv') ), caption = "Lateness by Brand and Category for December")
```
<br>
...
datatable(format(Mas_Active_Late, big.mark = ","), options = list(pageLength = 5), caption = "Lateness by Brand and Category for YTD" )
```
<br>
...
```{r echo=FALSE}
datatable(format(Mas_Intimates, big.mark =","), extensions ='Buttons'  , options = list(pageLength = 6, scrollX = TRUE, dom = 'Bfrtip' , buttons = c('excel', 'pdf', 'csv') ))
```
