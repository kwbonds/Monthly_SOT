...
```{r Libraries, warning=FALSE, error=FALSE, comment= FALSE, message=FALSE}
library(dplyr)
library(readr)
library(RODBC)
library(formattable)
...
```{r Prompt for week, eval=FALSE}
EOW <- prompt_for_week()
```
```{r Set static week, include=FALSE}
EOW <- 48
